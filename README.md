...
| `OPS_QA_MULTI_AGENT` | 全部 | `1/true/yes/on` 开启多 agent 编排，终端与飞书**共用**。终端可用 `--multi-agent` / `--no-multi-agent` 覆盖；飞书无命令行开关，只认此变量 |
...
也可用环境变量 `OPS_QA_MULTI_AGENT=1` 作为默认开关（与飞书共用一套 `.env`），命令行 `--multi-agent` / `--no-multi-agent` 仍可临时覆盖。
...
**答题模式**：飞书没有命令行开关，默认单 agent。要开多 agent 编排（分诊 → 组件专家），在 `.env` 里加 `OPS_QA_MULTI_AGENT=1` 即可；启动日志会回显当前模式（`答题模式：多 agent 编排（模型 …）`）。
