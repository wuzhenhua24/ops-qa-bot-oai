...
`single` / `multi` / `coordinator` 是**确定性模式**，主要给评测台按模式跑分对比用（差异化 #5）；日常交互用默认 `auto` 即可。终端 `--mode` 可临时覆盖环境变量。结构化输出（`--structured`）/ 护栏（`--guardrails`）只在 `single` 下生效，不与编排模式叠加。
...
跨 provider 用**非严格** schema 下发（`strict_json_schema=False`），兼容 Claude / 智谱 / 火山等不支持 OpenAI strict 结构化输出的端点。自由文本 + 标记的路径（`answer()` / 不带 `--structured`）仍保留，二者可按需选用。
...
REPL 交互式按 y/n 审批；一次性 `--ask` 模式无人值守，写操作**默认驳回**（安全）。`--guardrails` 与 `--structured` 叠加时额外启用输出来源护栏；仅 `single` 模式生效，不与编排模式（`auto`/`multi`/`coordinator`）叠加。实现见 `guardrails.py` / `actions.py` 与 `OpsQABot.answer_guarded()`。
