...
`single` / `multi` / `coordinator` 是**确定性模式**，主要给评测台按模式跑分对比用（差异化 #5）；日常交互用默认 `auto` 即可。终端 `--mode` 可临时覆盖环境变量。**护栏（`--guardrails`）是横切关注点，与任何 `--mode` 并存**（注入护栏挂入口 agent、写审批工具挂各专家）。结构化输出（`--structured`）走独立的单 agent 路径，只在 `single` 下生效。
