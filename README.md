...
│   ├── feedback_stats.py     # 反馈统计报表 CLI：读 logs/feedback.log 出离线报告（满意率/升级率/成本拆分）
│   ├── bot.py                # OpsQABot：Agent + Runner，answer()/answer_structured()/answer_guarded()
│   ├── cli.py                # 交互式 REPL + --ask/--structured/--mode/--guardrails
│   └── feishu/               # 飞书长连接接入：render（渲染纯逻辑）/ inbound（贴图/富文本解析）/ approvals（审批卡 HITL）/ archive（问答归档）/ feedback（反馈卡）/ followup（定时器）/ session / runner
├── eval/                     # 评测题集 cases.json（带 expected_decision/component/route）+ 实测快照 RESULTS.md
├── tests/                    # 确定性回归测试（无需 LLM）：检索/沙箱/契约/护栏/审批/诊断/DB/链路/复核/飞书各闭环，共 15 个文件
├── deploy/                   # 部署素材：systemd service + 部署说明
├── run.py                    # CLI 入口
├── run_eval.py               # 评测入口
├── run_ws.py                 # 飞书长连接入口
└── pyproject.toml
...
> `--mode coordinator` 是把协调者当**唯一入口**（确定性，评测用）。默认的 `auto` 模式下，协调者是分诊台的一个 handoff 目标——只有分诊判定问题跨组件时才升级给它，单组件问题仍走该组件自己的专家。
...
| `OPS_QA_MODEL_<组件目录大写>` | 某组件专家的模型，如 `OPS_QA_MODEL_REDIS`。键取 INDEX.md「目录」列（不是「组件」列显示名），大小写不敏感；目录名带连字符的设不出来（shell 不允许变量名带 `-`） |
| `OPS_QA_MODEL_COORDINATOR` | 跨组件协调者的模型（`coordinator` / `auto` 模式） |

**答题模式与路由的关系**：模式决定建出哪些角色（图的形状），路由决定每个角色用什么模型；覆盖只对当前模式实际建出的角色生效，没建的角色对应覆盖静默闲置（不报错，验证看启动横幅"模型路由"一行）。`single` 只用 `OPS_QA_MODEL`；`multi` 生效 triage + 组件；`coordinator` 生效 coordinator + 组件；`auto`（默认）三类全生效——注意 auto 下每个组件专家建了**两份**（分诊直接转交的 + 协调者内部 `ask_<组件>` 的），同一覆盖对两份同时生效。**复核者是例外**：`OPS_QA_REVIEWER_MODEL` 不受模式约束、任何模式（含 single）都生效——它是答完后的独立判定环节，不在编排图里。
...
无任何覆盖时所有角色都用 `OPS_QA_MODEL`，等价单模型。实现见 `ModelRouter` / `build_model_router`（`ops_qa_bot_oai/model.py`）。以上覆盖只换模型名（同一 provider/端点）；唯一支持**独立端点**（自己的 provider/URL/key）的角色是复核者（`OPS_QA_REVIEWER_PROVIDER`，见「二次复核」）——复核的价值来自判断独立性、常需跨模型家族，其余角色没有真实需求，不泛化。
...
```bash
# 缺省关；--db 临时开（等价 OPS_QA_DB=1）。未配只读账号时自动降级为模拟数据（标注来源），先把链路跑通：
...
**核心问答闭环**（当前范围）：群里 @机器人 → 立即发占位消息 → 跑 `OpsQABot.answer()` → 把占位**编辑**成最终答案（头部 @ 提问者；命中 `<<ESCALATE>>` 时末尾 @ 负责人）。会话按 `(chat_id, user_id)` 隔离、`/reset` 开新会话、`/tasks` 管理定时跟进、`/cancel`（或「取消」）停掉自己在途的提问、`/help`（或「帮助」）出能力清单与指令说明（组件清单现读 INDEX.md，可选特性按实际开关动态拼——没启用的不出现，避免照着帮助试被关掉的功能）、不支持的消息类型（file/sticker/audio…）回友好提示。同用户连发多条问题按到达顺序排队串行（per-key 锁），排队中的占位前缀是「🕒 排队中」、轮到开跑时刷成「🔍 翻文档中」，多条占位一眼分清哪条在跑；不同用户互不排队。实现见 `ops_qa_bot_oai/feishu/`，渲染纯逻辑（问题清洗 / 升级 open_id 解析 / @ 段拼装）已单测；真机运行需你的飞书凭证。
...
**答题模式**：飞书没有命令行开关，默认 `auto`（自适应分诊，见上「答题模式」）。要固定成别的模式，在 `.env` 里设 `OPS_QA_MODE=single|multi|coordinator` 即可；启动日志会回显当前模式（`答题模式：自适应分诊（单组件专家 / 跨组件协调）（模型 …）`）。
