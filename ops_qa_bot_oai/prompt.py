...
# / `<<ESCALATE>>` / `<<FOLLOWUPS>>` 的契约只写在它里面。多 agent 模式下真正答题的是分诊台
...
#
# 结构化模式**不用**它：那边走 AnswerContract 的 decision/escalate_to/followups 字段，
...
def free_text_markers_section(*, escalate_rule: str) -> str:
    """自由文本模式下三个答案塑形标记的契约。`escalate_rule` 由调用方按 agent 定制。"""
    return f"""

# 答案塑形标记（重要）

答完后按需在答案**末尾、各自独立一行**输出下面的标记。系统会解析并把标记本身从展示文本里剥掉（用户看不到它们），据此渲染 @负责人 / 识别反问轮 / 生成追问按钮。**格式必须精确**：不改空格、不加引号、不要放进代码块。
...
- **绝不要**与 `<<ESCALATE:...>>` / `<<FOLLOWUPS:...>>` 同时出现（反问 ≠ 找不到）。
...
## `<<FOLLOWUPS:k1|k2|k3>>` —— 快捷追问按钮（最多 3 个，可以不写）

- 可选 key（**严格使用这些 slug**）：`troubleshoot`（排查步骤）/ `risks`（风险与注意事项）/ `rollback`（回滚方案）/ `checklist`（可勾选清单）/ `commands`（示例命令）/ `related`（相关文档）。
- 故障/异常类问题 → `troubleshoot|risks|commands`；变更/操作类 → `rollback|risks|commands`；信息/定义类通常不挂。
- 反问 / 升级 / 拒绝 / 问候场景 → **不输出本标记**。不挂时整个标记都不写，不要写空标记 `<<FOLLOWUPS:>>`。
- 放在最末尾（若同时有 `<<ESCALATE:...>>`，排在它之后）。"""


# 分诊台只做路由，不答组件问题，所以三个标记里只可能用到 `<<CLARIFY>>`（问"你主要问哪个
# 组件"那一轮）。给它完整契约反而会诱导它自己升级/挂追问按钮，抢了专家的活。
TRIAGE_CLARIFY_NOTE = """
