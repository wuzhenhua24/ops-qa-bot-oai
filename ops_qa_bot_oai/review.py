...
# 输出格式（严格遵守）
只输出**一个 JSON 对象**，不要代码围栏、不要任何解释文字。形如：
{"verdict": "approve", "findings": [], "grounded": true}
或：
{"verdict": "revise", "findings": ["答案说 maxmemory 默认 8gb，但 redis/overview.md 未提及该默认值"], "grounded": false}"""
...
async def run_review(
    ...
    try:
        ...
        # 输出不符合 ReviewVerdict 契约（典型：端点不支持/忽略 json_schema，模型输出
        # `verdict: approve` 这类 YAML 风格纯文本）。与"调用失败"分开报——这类失败每次
        # 必现，复核等于被静默旁路，该换 reviewer 模型或端点；别让排查方向跑偏到鉴权上。
        logger.warning(
            "reviewer 输出不符合 ReviewVerdict 契约（该端点可能不支持 JSON 结构化输出，"
            "建议换 reviewer 模型/端点），fail-open（视作 approve）：%s",
            str(e)[:200],
        )
        return None
