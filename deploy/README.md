...
# 3. 准备代码目录
sudo mkdir -p /opt/ops-qa-bot-openai
sudo chown ops-bot:ops-bot /opt/ops-qa-bot-openai
sudo -u ops-bot git clone <仓库地址> /opt/ops-qa-bot-openai
cd /opt/ops-qa-bot-openai
...
sudo -u ops-bot mkdir -p /opt/ops-qa-bot-openai/data
```
...
```bash
sudo mkdir -p /etc/ops-qa-bot-openai
sudo tee /etc/ops-qa-bot-openai/ops-qa-bot-openai.env >/dev/null <<'EOF'
# ── 飞书应用凭证（必填）──
...
OPS_QA_SESSION_DB=/opt/ops-qa-bot-openai/data/.sessions.db
...
sudo chown root:ops-bot /etc/ops-qa-bot-openai/ops-qa-bot-openai.env
sudo chmod 640 /etc/ops-qa-bot-openai/ops-qa-bot-openai.env
```
...
**文档内容**：机器人答的是 `/opt/ops-qa-bot-openai/docs/` 下的文档 + `INDEX.md` 路由表。
...
```bash
sudo cp /opt/ops-qa-bot-openai/deploy/ops-qa-bot-openai.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now ops-qa-bot-openai

# 检查
sudo systemctl status ops-qa-bot-openai
sudo journalctl -u ops-qa-bot-openai -f      # 实时日志
```
...
会话历史：落盘 /opt/ops-qa-bot-openai/data/.sessions.db（重启/回收后可恢复上下文）
...
```bash
# liveness：进程在不在
sudo systemctl is-active ops-qa-bot-openai        # active / failed，给监控脚本用

# 最近有没有报错
sudo journalctl -u ops-qa-bot-openai -p err --since "10m ago" --no-pager
```

**P0-1 兜底已内建**：单条问题处理时若模型服务异常/超时/网络抖动，机器人会把占位消息
编辑成「⚠️ 出错了，请稍后重试」并记录堆栈到 journald，**不会**让用户对着「🔍 翻文档中」
干等。用 `journalctl -u ops-qa-bot-openai | grep "answer failed"` 可统计这类失败频率。
...
# 状态 / 重启 / 停止
sudo systemctl status ops-qa-bot-openai
sudo systemctl restart ops-qa-bot-openai
sudo systemctl stop ops-qa-bot-openai

# 日志
sudo journalctl -u ops-qa-bot-openai -f                 # 实时
sudo journalctl -u ops-qa-bot-openai --since "1h ago"   # 最近 1 小时
sudo journalctl -u ops-qa-bot-openai -p err             # 仅 ERROR
...
- 看 `journalctl -u ops-qa-bot-openai --no-pager` 最后 50 行
- 常见：EnvironmentFile 路径错、`FEISHU_APP_ID/SECRET` 没填、模型 key/端点填错、
  忘了 `uv sync --extra feishu`（报 `缺少 lark-oapi`）

**反复重启（触发 StartLimit 后停手）**
- `journalctl -u ops-qa-bot-openai -p err --since "10m ago"` 找根因
- 修完：`sudo systemctl reset-failed ops-qa-bot-openai && sudo systemctl restart ops-qa-bot-openai`
...
想换路径（如装到 `/srv/...`），只改 `ops-qa-bot-openai.service` 里这几个字段：
...
改完 `sudo systemctl daemon-reload && sudo systemctl restart ops-qa-bot-openai`。
